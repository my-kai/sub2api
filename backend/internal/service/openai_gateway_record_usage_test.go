...
func TestRecordCyberPolicyUsageLog_BillsRealUpstreamTokens(t *testing.T) {
	...
	require.InDelta(t, expected.ActualCost, billingRepo.lastCmd.BalanceCost, 1e-12, "按真实 token 进入统一扣费仓储，与 WS/正常请求一致")
	require.Equal(t, 0, userRepo.deductCalls, "统一扣费仓储可用时不走旧 userRepo 兜底扣费")
}
