    ...
    <div v-if="!isSimple" class="card p-4">
      <div class="flex min-w-0 items-start gap-3">
        ...
        <div class="min-w-0 flex-1">
          ...
          <p class="mt-1 text-xl font-bold leading-tight text-emerald-600 dark:text-emerald-400">
            ${{ formatBalance(availableBalance) }}
          </p>
          <div class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs" :title="balanceBreakdownTooltip">
            <span class="text-gray-500 dark:text-gray-400">{{ t('dashboard.availableBalance') }}</span>
            <span
            ...
            >
              {{ t('dashboard.giftBalance') }} ${{ formatBalance(giftBalance) }}
            </span>
          </div>
        </div>
      </div>
...
const giftBalanceTooltip = computed(() => `${t('dashboard.giftBalance')}: $${formatBalance(props.giftBalance)}`)
const balanceBreakdownTooltip = computed(
  () => `${t('dashboard.currentBalance')}: $${formatBalance(props.balance)} / ${giftBalanceTooltip.value}`
)
