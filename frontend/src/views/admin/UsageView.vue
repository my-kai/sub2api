...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('admin.usage.requestId'), t('admin.usage.upstreamRequestId'), t('usage.userAgent'), t('admin.usage.ipAddress'), t('admin.usage.egressHost')
    ]
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.request_id || '', log.upstream_request_id || '', log.user_agent || '', log.ip_address || '', log.egress_host || ''
      ])
      if (rows.length) {
...
// Column visibility
const ALWAYS_VISIBLE = ['user', 'created_at', 'egress_host']
...
const allColumns = computed(() => [
  ...
  { key: 'egress_host', label: t('admin.usage.egressHost'), sortable: false }
])
